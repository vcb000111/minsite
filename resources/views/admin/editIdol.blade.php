...
@section('content')
@section('title', 'MinSite - Edit IDOL')
<div class="container-fluid bg-gray-200 p-4 border border-bottom-primary border rounded">

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <!-- Content Row -->
            <h4 class="text-center font-weight-bold mb-4 text-primary">Edit IDOL</h4>
            ...
            <form method="POST" class="mt-3" enctype="multipart/form-data" action="{{ route('admin.idols.edit.post', $idol->id) }}">
                @csrf
                <div class="row">
                    <div class="col-6">
                        <input type="text" autofocus value="{{ $idol->idol_name }}" class="form-control" placeholder="IDOL name" required name="idol_name">
                    </div>
                    <div class="col-6">
                        <input type="text" class="form-control " value="{{ $idol->idol_thumbnail }}" placeholder="IDOL thumbnail" required name="idol_thumbnail">
                    </div>
                </div>
                <button type="submit" class="btn btn-primary btn-block mt-2">Edit IDOL</button>
            </form>
        </div>
    </div>
</div>
